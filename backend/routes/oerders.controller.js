...
router.post('/add/:id', auth, checkAccess([ROLES.USER, ROLES.SELLER]), async (req, res) => {
    try {
        ...
        // Add product to orders if not already in orders
        if (!user.orders.includes(id)) {
            user.orders.push(id);
            await user.save();
        }

        res.status(200).json({ message: "Product added to orders", orders: user.orders });
    } catch (err) {
...
// Remove a product from the orders
router.delete('/delete/:id', auth,checkAccess([ROLES.USER, ROLES.SELLER]), async (req, res) => {
    try {
        const user = await userModel.findById(req.user._id);
        const id = req.params.id;

        user.orders = user.orders.filter(item => item.toString() !== id);
        await user.save();

        res.status(200).json({ message: "Product removed from orders", orders: user.orders });
    } catch (err) {
        res.status(500).json({ message: err.message });
    }
});
